...
<x-admin-layout>
    <section class="flex flex-col items-center justify-center text-whit min-h-[calc(100vh-var(--header-height))]">
        @if ($canResister)
        ...
        @if (count($quizzes) > 0)
            <ul class="grid grid-cols-9 gap-4 mb-12 text-white w-full max-w-4xl sm:grid-cols-9">
                <!-- ヘッダー -->
                ...
                @foreach ($quizzes as $quiz)
                    ...
                    <!-- 編集・削除（スマホ時だけ横並び） -->
                    <li class="mx-auto col-span-9 sm:col-span-2">
                        {{-- 編集ページに遷移 --}}
                        <button type="button"
                            ...
                            class="h-[30px] bg-white text-blue-900 px-2 py-1 rounded flex-none w-[60px] md:flex-1 ransition duration-300 hover:opacity-50">
                            編集
                        </button>
                        ...
                        <button
                            ...
                            class="flex-none w-[60px] ml-2 md:flex-1 bg-white text-red-500 px-2 py-1 rounded ransition duration-300 hover:opacity-50">
                            削除
                        </button>

                        <x-modal {{-- id,message,onConfirm(削除実行ロジック)を設定 --}} :id="'deleteModal' . $quiz->id" message="本当に削除しますか？" :onConfirm="'document.getElementById(\'deleteForm' . $quiz->id . '\').submit()'" />
        ...
        @else
            <p class="mb-12">まだクイズが登録されていません</p>
        @endif

        <button type="button" onclick="location.href='{{ route('admin.top') }}'"
